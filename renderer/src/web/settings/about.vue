<template>
  <div class="p-2 flex flex-col h-full items-center">
    <div class="flex flex-col items-center p-2 mb-4">
      <img class="w-28 h-28" src="/images/PoE2Overlay.png">
      <p class="text-base" v-if="false">Awakened PoE Trade</p>
      <p class="text-base">PoE2 Overlay</p>
      <p class="">{{ t('app.version', [version]) }}</p>
      <p>Simple overlay tool based on Awakened PoE Trade.</p>
      <p>Awakened PoE Trade is developed by Alexander Drozdov (<a href="https://github.com/SnosMe/awakened-poe-trade/releases">SnosMe</a>)</p>
    ...
    <div class="border border-gray-600 rounded p-2 whitespace-nowrap min-w-min w-72" v-if="false">
      <p>{{ info.str1 }}</p>
      <p>{{ info.str2 }}</p>
      <button v-if="info.action" @click="info.action" class="btn w-full mt-1">{{ info.actionText }}</button>
    </div>
    <div class="text-center mt-auto py-8" v-if="false">
    ...
    <div class="text-center mt-auto py-8">
      <p>Adapted for PoE2 by Popa Razvan Dragos (<a href="https://github.com/poparazvandragos/PoE2Overlay">KiLLeR13</a>)</p>
    </div>
  </div>
</template>
